...
<head>
    ...
    <title>Cetak Laporan Surat Keluar - SMKN 2 Padang</title>
    <style>
        /* 1. Pengaturan Kertas HVS (A4) */
        @media print {
            @page {
                size: A4;
                margin: 1.5cm 1cm 1.5cm 2cm; /* Margin kiri 2cm buat ruang jilid */
            }
        ...
        .logo-box img {
            width: 75px; /* Pastikan file logo.png ada di folder public/ */
            height: auto;
        }
        ...
        table {
            ...
            table-layout: fixed; /* Mencegah tabel meluber keluar kertas */
        }
...
<body onload="window.print()">
    ...
    <div class="info-rekap">
        ...
        <strong>TOTAL:</strong> {{ count($data) }} Surat Keluar
    </div>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th width="120px">Tujuan Surat</th> <th width="80px">Tgl. Surat</th>
        ...
        <tbody>
            @forelse($data as $index => $letter)
                <tr>
                    ...
                    <td>{{ $letter->to }}</td> <td class="text-center">{{ \Carbon\Carbon::parse($letter->letter_date)->format('d/m/Y') }}</td>
                    <td>{{ $letter->description }}</td>
                    <td class="text-center">{{ $letter->user->name ?? 'System' }}</td>
                </tr>
            @empty
                <tr>
                    <td colspan="7" class="text-center">Data agenda surat keluar tidak ditemukan.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
