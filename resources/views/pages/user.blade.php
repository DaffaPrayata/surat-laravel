...
{{-- ALERT PASSWORD (Fungsi Original Lu Jaga) --}}
        ...
        <table class="table table-hover align-middle">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><span class="badge bg-label-info">{{ ucfirst($user->role) }}</span></td>
                    <td><span class="badge {{ $user->is_active ? 'bg-label-success' : 'bg-label-danger' }}">{{ $user->is_active ? 'Aktif' : 'Nonaktif' }}</span></td>
                    <td class="text-center">
                        @if(auth()->user()->role === 'admin')
                        <div class="d-flex justify-content-center gap-2">
                            <button class="btn btn-sm btn-info btn-edit" data-id="{{ $user->id }}" data-name="{{ $user->name }}" data-email="{{ $user->email }}" data-phone="{{ $user->phone }}" data-role="{{ $user->role }}" data-active="{{ $user->is_active }}" data-bs-toggle="modal" data-bs-target="#editModal">
    ...
    <div class="mobile-card shadow-sm">
        <div class="d-flex align-items-center mb-3 border-bottom border-secondary pb-2">
            ...
            <div class="flex-grow-1">
                <div class="fw-bold text-white">{{ $user->name }}</div>
                <span class="badge bg-label-info small">{{ ucfirst($user->role) }}</span>
            </div>
            @if(auth()->user()->role === 'admin')
            <div class="dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-end dropdown-menu-dark">
                    <button class="dropdown-item btn-edit" data-id="{{ $user->id }}" data-name="{{ $user->name }}" data-email="{{ $user->email }}" data-phone="{{ $user->phone }}" data-role="{{ $user->role }}" data-active="{{ $user->is_active }}" data-bs-toggle="modal" data-bs-target="#editModal">Edit User</button>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <form action="{{ route('user.store') }}" method="POST" class="modal-content">
            ...
            <div class="modal-body row g-3">
                ...
                <div class="col-md-6"><label class="form-label small fw-bold text-uppercase">Role</label><select name="role" class="form-select"><option value="admin">Admin</option><option value="staff" selected>Staff</option></select></div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <form method="POST" id="editForm" class="modal-content">
            ...
            <div class="modal-body row g-3">
                ...
                <div class="col-md-6"><label class="form-label small fw-bold text-uppercase">Role</label><select id="edit_role" name="role" class="form-select"><option value="admin">Admin</option><option value="staff">Staff</option></select></div>
